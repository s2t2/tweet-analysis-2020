...
class BigQueryService():
    ...
    def fetch_daily_active_user_friends(self, date, tweet_min=4, limit=None):
        ...
        sql = f"""
            ...
            JOIN `{self.dataset_address}.user_tweet_rates` tr ON tr.user_id = u.user_id
            LEFT JOIN `{self.dataset_address}.2_bot_communities` b ON b.user_id = u.user_id
        """
        if limit:
            sql += f" LIMIT {int(limit)} "
        return self.execute_query(sql)
    ...
    def fetch_daily_active_user_friends_v2(self, date, tweet_min=5, limit=None):
        ...
        sql = f"""
            ...
            JOIN {self.dataset_address}.active_user_friends_flat uff ON uff.user_id = dau.user_id
            JOIN {self.dataset_address}.active_user_friends_flat uff2 ON uff.friend_name = dau.screen_name
