...
# HYPERPARAMS
MU = os.getenv("MU", default="OOPS")
alpha1 = float(os.getenv("ALPHA1", default="OOPS"))
alpha2 = float(os.getenv("ALPHA2", default="OOPS"))
iterations = int(os.getenv("N_ITERS", default="OOPS"))
db = os.getenv("TOPIC_NAME", default="impeachment")
mode = os.getenv("MODE", default="OOPS")
alambda1 = float(os.getenv("LAMBDA1", default="0.8"))
alambda2 = float(os.getenv("LAMBDA2", default="0.6"))
epsilon = 10**(-float(os.getenv("EPSILON", default="OOPS")))
SEED = int(os.getenv("SEED", default="0.6"))

alpha=[MU,alpha1,alpha2]


retweet_graph = load_graph() #readCSVFile_G('RT_graphs/'+db+'_G0_RT_GRAPH.csv')

subgraphs_dirpath = os.path.join() # './'+db+'_subGraphs'
pibots_dirpath = os.path.join() # './network_piBots_'+db

if not os.path.exists(subgraphs_dirpath):
    os.makedirs(subgraphs_dirpath)

if not os.path.exists(pibots_dirpath):
    os.makedirs(pibots_dirpath)

##split all users in batches
all_users = sorted(list(retweet_graph.nodes()))
...
if(rank==nproc-1):
	...
	while(True and countP < iterations):
		...
		inDeg = retweet_graph.in_degree(weight='weight')
		if(type(inDeg)!=dict):
			graph_in = dict((x,y) for x, y in inDeg)
		else:
			graph_in = inDeg

		outDeg = retweet_graph.out_degree(weight='weight')
		if(type(outDeg)!=dict):
		...
		link_data = getLinkDataRestrained(retweet_graph)

		for n in retweet_graph.nodes():
		...
		H, PL, user_data = computeH(retweet_graph, piBot, edgelist_data, graph_out, graph_in)
