...
class DailyFriendGrapher(FriendGraphStorage, Job):
    def __init__(self, bq_service=None, date=DATE, tweet_min=TWEET_MIN, batch_size=BATCH_SIZE, limit=LIMIT):
        ...
        Job.__init__(self)
        FriendGraphStorage.__init__(self, dirpath=f"active_tweeter_friend_graphs_v2/{self.date}/tweet_min/{self.tweet_min}")
    ...
    def fetch_nodes(self):
        ...
        for row in self.bq_service.fetch_daily_active_user_friends_v2(date=self.date, tweet_min=self.tweet_min, limit=self.limit):
    ...
    def nodes_df(self):
        ...
        del self.nodes
        lower_max = quantile(df["mean_opinion_score"], 0.05) #> ~0
        upper_min = quantile(df["mean_opinion_score"], 0.95) #> ~1
        print("LOWER:", lower_max, "| UPPER:", upper_min)

        df["stubborn"] = 1 * logical_or(df["mean_opinion_score"] <= lower_max, df["mean_opinion_score"] >= upper_min) # 1 if below the low or above the high, else 0
