...
def G_from_follower_graph(node_filename, follower_graph_filename, threshold_low, threshold_high):
    G = nx.DiGraph()
    print("Building network for Assess.\nStubborn intervals = (0,%.3f),(%.3f,1)" % (
        threshold_low, threshold_high))
    # first get the nodes and their info and add it to the graph object G
    data_nodes = pd.read_csv(node_filename)
    data_nodes.Stubborn = 1*np.logical_or(data_nodes.InitialOpinion <=
                                          threshold_low, data_nodes.InitialOpinion >= threshold_high)
    for row in data_nodes.iterrows():
        G.add_node(row[1]['id'], Name=row[1]['id'], InitialOpinion=row[1]['InitialOpinion'], Stubborn=row[1]['Stubborn'],
                   Rate=row[1]['Rate'], FinalOpinion=row[1]['InitialOpinion'], Bot=row[1]['Bot'])
...
def final_opinions(Ginitial):
    ...
    # create the matrices we need for the opinion calculation.
    (Gmat, Fmat, Psi) = graph_to_GFPsi(G)
    ...
    # solve linear system to get non-stubborn opinions
    opinion_nonstubborn = sparse.linalg.bicgstab(Gmat, b)[0]
    cnonstub = 0
    ...
    for node in G.nodes(data=True):
        if node[1]['Stubborn'] == 0:
            ...
            if opinion_nonstubborn[cnonstub] > 1:
                print("%s has opinion %s - not between 0 and 1" %
                      (node, opinion_nonstubborn[cnonstub]))
            cnonstub += 1
    FinalOpinions = [x[1]['FinalOpinion']
                     for x in G.nodes(data=True)]  # create a FinalOpinions list
    # return the Final opinions as an array and also return the update graph object
    return (np.asarray(FinalOpinions), G)
...
def graph_to_GFPsi(G):
    ...
    for edge in G.edges(data=True):
        ...
        if follower_stub == 0 and following_stub == 0:  # add an edge to Gmat because both non-stubborn
            ...
            data_F.append(rate)
    Gmat = sparse.csr_matrix((data_G, (row_G, col_G)),
                             shape=(n_nonstubborn, n_nonstubborn))
    Fmat = sparse.csr_matrix((data_F, (row_F, col_F)),
                             shape=(n_nonstubborn, n_stubborn))
...
def risk_index(Gbot0):
    Gnobot = Gbot0.subgraph(
        [x for x in Gbot0.nodes if Gbot0.nodes[x]["Bot"] == 0])
...
def bot_neighbor_count(G):
    ...
    for node in G.nodes():
        if G.nodes[node]["Bot"] == 1:
            nbots += 1
            nb = G.neighbors(node)
            Vbot_followers = Vbot_followers + \
                [v for v in nb if G.nodes[v]["Bot"] == 0]
